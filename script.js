...
(()=>{
  ...
  const summaries={
    'case-study-ad-engine.html':{
      ...
      solution:'I designed an n8n automation connecting AI-generated ad copy, Meta campaign actions, Google Sheets logging, webhook lead capture, and follow-up.',
      thought:'I approached it like a support escalation: identify every handoff, find where effort or failure accumulates, then connect the steps without hiding operator visibility.',
    ...
    'case-study-agent-orchestration-lab.html':{
      ...
      solution:'A mission-control prototype with capability routing, least-privilege tool permissions, fallback recovery, guardrails, execution traces, run history, and observability.',
      thought:'I treated orchestration as an operations problem: define responsibility, constrain access, inject controlled failures, expose the trace, and verify recovery.',
      result:'The lab reproducibly demonstrates normal execution, simulated tool failure with approved fallback recovery, and blocked permission violations.'
    },
    'case-study-gpu-fleet-lab.html':{
      problem:'GPU workload health, capacity, topology, and scheduling decisions are difficult to reason about when scattered across infrastructure layers.',
      solution:'A GPU resource lab combining telemetry, DCGM-style metrics, NVLink/topology awareness, scheduling explanations, persistent history, fault simulation, and benchmarking.',
      thought:'Gather the signals first, establish relationships, make placement decisions explainable, and preserve enough history to diagnose outcomes.',
      result:'The working systems lab converts infrastructure signals into visible health, capacity, and scheduling decisions.'
    },
    'case-study-prompt-reliability-lab.html':{
      problem:'A prompt that works in one demo does not prove reliable behavior across repeated runs.',
      solution:'An evaluation lab for structured prompt tests, expected behavior, failure inspection, and repeatable comparison.',
      thought:'I reframed prompt engineering as quality engineering: define good behavior, test it, surface failures, and make change measurable.',
      result:'Prompt performance becomes observable and testable instead of dependent on one-off manual prompting.'
    },
    'case-study-rippro.html':{
      problem:'DTF/DTG operators often jump between artwork cleanup, presets, calibration, printers, queues, hot folders, and job history.',
      solution:'Crusoe RipPro Studio centralizes AI-assisted print preparation and production operations in one desktop workflow.',
      thought:'Map the operator’s real sequence, reduce context switching, and surface the right controls at the moment they matter.',
      result:'The prototype reorganizes a fragmented print-production workflow into one coherent operator experience.'
    },
    'case-study-mockup-magic.html':{
      problem:'Apparel sellers often need separate tools and repeated manual work for mockups, lifestyle imagery, ads, and exports.',
      solution:'Mockup Magic connects realistic apparel compositing, fictional AI models, campaign creative, and export organization.',
      thought:'Preserve creative control while reducing repetitive production steps and maintaining visual consistency from artwork to market.',
      result:'The product concept consolidates multiple content-production stages around the needs of apparel and print businesses.'
    },
    'case-study-video-engine.html':{
      problem:'Generative-video automation can break when model output is inconsistent or difficult for downstream steps to consume.',
      solution:'An n8n pipeline that generates structured video-script data, validates machine-readable output, and passes it to video generation.',
      thought:'Treat the model as one component in a system: constrain the output contract and validate before downstream use.',
      result:'The prototype demonstrates a controlled prompt-to-video handoff using structured outputs rather than free-form text.'
    },
    'case-study-credit-rise.html':{
      problem:'Credit improvement can involve scattered scores, negative accounts, disputes, education, and planning without a clear next step.',
      solution:'A user-centered dashboard combining bureau information, error discovery, dispute progress, planning tools, and clearly labeled illustrative scenarios.',
      thought:'Start with user anxiety and overload: prioritize clarity, progress, education, and the next useful action.',
      result:'The prototype organizes a complicated credit-improvement journey into a clearer task-oriented experience.'
    },
...
(()=>{
  ...
  const projects={
    ...
    'case-study-ai-security-lab.html':{id:'AA-AILAB-SEC-009',title:'AI Security & Adversarial Testing Lab'}
  };
  const file=(location.pathname.split('/').pop()||'index.html').toLowerCase();
